...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate CGO (table 1).')
    ...
    hyp_path = os.path.join(args.result_dir, 'hyp.json')
    table1_path = os.path.join(args.result_dir, 'table_1.json')
    ...
    if os.path.exists(hyp_path):
        ...
        with torch.no_grad():
            for cap_det_label in tqdm(loader_cap_detection):
                ...
                obj_pred = classifier(classifier_inputs)
                obj_pred = nn.Sigmoid()(obj_pred)
                top_obj_pred = torch.argsort(obj_pred,
                                             descending=True)[0, :10].tolist()
                image_features = classifier_inputs[0]
                top_k_hypothesis = cgo_decode_list(image_features,
                                                   top_obj_pred,
                                                   category_names, word_map,
                                                   reverse_word_map, lstm_left,
                                                   lstm_right, args.beam)
                hyp[image_id] = {'top_k': top_k_hypothesis}

                cap_det_gt = torch.nonzero(
                    cap_det_label['label'][0].view(-1)).view(-1).tolist()
                cap_det_gt_hypothesis = cgo_decode_list(
                    image_features, cap_det_gt, category_names, word_map,
                    reverse_word_map, lstm_left, lstm_right, args.beam)
                hyp[image_id]['cap_det_gt'] = cap_det_gt_hypothesis

            for det_label in tqdm(loader_detection):
                image_id = str(det_label['image_id'][0])
                image_features = det_label['inputs'][0].to(device)
                det_gt = torch.nonzero(
                    det_label['label'][0].view(-1)).view(-1).tolist()
                det_gt_hypothesis = cgo_decode_list(image_features, det_gt,
                                                    category_names, word_map,
                                                    reverse_word_map,
                                                    lstm_left, lstm_right,
                                                    args.beam)
                hyp[image_id]['det_gt'] = det_gt_hypothesis
    ...
    title = 'Caption GT label'
    logger.info('computing metrics for {0}'.format(title))
    avg_num = 0
    true_positive = [0 for _ in category_names]
    det_gt_positive = [0 for _ in category_names]
    for image_id in tqdm(hyp):
        img_hyp = ref[image_id]
        hit_set = set([])
        for sentence in img_hyp:
            hit = sentence_obj_hit(sentence, category_names)
            hit_set = hit_set.union(hit)
    ...
    det_gt_positive = [0 for _ in category_names]
    for image_id in tqdm(hyp):
        img_hyp = hyp[image_id]['cap_det_gt']
        k_hyp += [' '.join(x) for x in img_hyp]
        img_ref = [' '.join(x) for x in ref[image_id]]
        if len(img_ref) != 5:
            logger.warning('caption reference more than 5! ({0})'.format(
    ...
    det_gt_positive = [0 for _ in category_names]
    for image_id in tqdm(hyp):
        if not 'det_gt' in hyp[image_id]:
            continue
        img_hyp = hyp[image_id]['det_gt']
        k_hyp += [' '.join(x) for x in img_hyp]
        img_ref = [' '.join(x) for x in ref[image_id]]
        if len(img_ref) != 5:
            logger.warning('caption reference more than 5! ({0})'.format(
